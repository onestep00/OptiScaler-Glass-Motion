...
extern "C"
{
    ...
    // The first evaluate export, kept for hosts that predate v2 -- the native Vulkan pass among them. It
    // has one guide extent for depth and motion vectors, both at the origin, which is exactly what v2 is
    // given here, so the two cannot drift apart.
    ...
    {
        return dlssnr_vk_evaluate_v2(cmdBuffer, feature, capabilityParams, color, depth, motion, output, width, height,
                                     guideWidth, guideHeight, guideWidth, guideHeight, 0, 0, 0, 0, depthInverted, reset,
    ...
    // The first evaluate export, kept so a host built before v2 still runs against this forwarder. It has
    // one guide extent for depth and motion vectors, both at the origin, which is exactly what v2 is given
    // here. v2 makes the call into the snippet, so the caller gate still sees this module.
    __declspec(dllexport) int dlssnr_call_evaluate(ID3D12GraphicsCommandList* cmd, void* feature,
                                                   ...
                                                   float skinStructure, int useAutoMask, float mvScaleX, float mvScaleY)
    {
        return dlssnr_call_evaluate_v2(cmd, feature, capabilityParams, color, depth, motion, output, width, height,
                                       guideWidth, guideHeight, guideWidth, guideHeight, 0, 0, 0, 0, depthInverted,
                                       reset, intensity, style, localStructure, localTone, skinStructure, useAutoMask,
                                       mvScaleX, mvScaleY);
    }
