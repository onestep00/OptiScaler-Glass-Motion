...
- `audit_motion_writer_slots.py`: exact VS/PS metadata union and local native constructor/write-range inspection; preserves unresolved suppliers and never grants runtime admission. Writes the direct-span-clear subset `shader-pairs-<n>-direct-span-clear.bin`, the only pairs `Glass/motion-shader-pairs.bin` may copy.
...
- `export_motion_declarations.py`: pending bounded declarations and exact VS/PS cache pairs of transparency-route techniques and of `renderstage_hair_basecolor_blend` (blended hair PSOs the packed capture admits with the VS's root graft); records unpaired variants, no automatic deployment or writer/history admission.
- `export_native_grafts.py`: validated grafts to the local `Glass/grafts` catalog (`GGRAFT02`: root outputs, supply class from the previous-graph union, camera-only variant outputs or 0xFFFFFFFF; camera-only records have root outputs 0xFFFFFFFF, no `<sha>.dxil`, and the target's own current-position class); output is extracted game code and stays in the ignored `artifacts/` tree. A vehicle target (`vehicle` in index.json) gets no camera-only record: it is listed in `refused.bin` (`GGREFS01`, reason 1) and its draws keep the engine's motion. `light-ps.bin` (`GGLTPS01`) lists the PS whose census passes (`all-cache-techniques.json`) are all light passes (transparent, unlit, screen-space VFX); only these get the brightness term of the packed record opacity. `background-ps.bin` (`GGBGPS01`, same layout) lists the PS whose displayed pixel is background content: every `renderstage_distortion` PS, the `screenspace_vfx` PS of `cloak*`/`optical_camouflage*` materials and every PS of `global_water_patch`/`water_plane`; the module compiles their packed variants coverage-only.
