...
- PackedMotion.cpp/HLSL checks SM 6.6 root-UAV 64-bit atomic nearest-layer arbitration on an independent NVIDIA device. GeometryInstances `--packed` and `--packed-mrt` check the rewritten material in real single/independent-MRT graphics draws while preserving original color/depth/discard. `--packed` also draws an additive (ONE/ONE) variant of the same material, compiled as a light-pass PS, at three emission scales and checks that its record weight and covered class follow saturate(luma(F) * scale) with material opacity 0; the same variant compiled for a PS outside the light passes records weight 0, uncovered, at scale 2.2. Neither measures game cost or connects the packed result to FG.
...
- NativeSession.cpp runs synthetic inputs on independent DIRECT/COMPUTE queues. It checks rejected admission, unsubmitted/in-flight retirement and the DLSS-NR list hold (NrMotion off, unsubmitted, resubmittable, in flight, destroyed), not glass quality. Its artificial GPU gate and CPU drains are test-only. A trailing `--warp` runs it on the WARP adapter.
...
- NativeGraftPacked.cpp checks the catalog lookup and the packed native-graft VS/PS rewrite (no history resources, linked graft rows, no jitter term) through DXC assembly and validation on one exported graft and its camera-only array variant (root reads b7[24..26]; camera variant reads no b7[24..26] and the native previous camera rows). Its `light`/`nonlight` mode checks the light-ps.bin classification of one original PS and that the packed rewrite adds the brightness term (%glass.emission, %glass.opacity) only for a light-pass PS and otherwise records %glass.alpha itself; every PS rewrite takes lightTarget from the catalog. Offline; no device, PSO or game.
- PackedMotionSelection.cpp checks the second consumer's frame selection and includes the production PackedMotionCapture.cpp with owned fixture seams. Two recording threads admit every draw while each waits inside identity resolution for the other (this fails when resolution runs under the capture mutex); concurrent reservations stay disjoint; a refused pipeline's gates land on its own GEOMETRY_PIPELINE split; a 100-element draw admits across two element batches into one reservation. Independent device; no game draws, GPU execution or cost claim.
...
- [VertexHistoryCache.md](VertexHistoryCache.md): PackedMotionMappings checks separate chunk histories, shared object boundary IDs and distinct array lifetimes. Local capture prototype compiles; native identity/queue admission and game MV remain incomplete.
...
- [GeometryObjects.md](GeometryObjects.md): Actual array setter invalidates lifetime during mutation; native non-grouped source indices, nested/reused scopes and ten-function relocation pass owned checks. GPU publication and game MV remain incomplete; source only.
