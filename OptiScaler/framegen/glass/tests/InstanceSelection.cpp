...
int main()
{
    ...
    require(view.resolveGlobalPacket(true, 20160, 40, 20160, 40, 0));
    require(view.originalIndex(39, actual, read) && actual == 39);
    require(view.resolveGlobalPacket(true, 20000 + 32767, 32767, 20000, 65536, 0));
    require(view.originalIndex(0, actual, read) && actual == 32767);
    require(view.originalIndex(32766, actual, read) && actual == 65533);
    require(view.resolveGlobalPacket(true, 20000 + 65534, 2, 20000, 65536, 0));
    require(view.originalIndex(1, actual, read) && actual == 65535);
    require(!view.originalIndex(2, actual, read));
    require(!view.resolveGlobalPacket(false, 20160, 40, 20160, 40, 0));
    require(!view.resolveGlobalPacket(true, 20159, 40, 20160, 40, 0));
    require(!view.resolveGlobalPacket(true, 20161, 40, 20160, 40, 0));
    require(!view.resolveGlobalPacket(true, 131071, 2, 131071, 2, 0));
    require(!view.resolveGlobalPacket(true, 20000, 32768, 20000, 65536, 0));
    require(calls == previousCalls);
    // A group can repack the same range as [17, 3, ...]. Offset zero must
    // never become source zero merely because allocation/count stayed equal.
    require(!view.resolveGlobalPacket(true, 20160, 40, 20160, 40, 0x2000));
    require(!view.originalIndex(0, actual, read) && actual == UINT32_MAX);
    require(!view.resolveGlobalPacket(true, 20160, 40, 20160, 40, UINT32_MAX));
    require(CyberpunkInstanceSelection::globalStorageRange(true, 20160, 40, 20160, 40, actual) && actual == 0);
    require(!CyberpunkInstanceSelection::globalStorageRange(true, 20159, 40, 20160, 40, actual) && actual == UINT32_MAX);
    require(calls == previousCalls);
    std::cout << "PASS reordered40=1 culled1=1 range_rejection=1 setup_reads=3 setup_bytes=16 "
                 "per_instance_bytes=2 global_packet_splits=1 grouped_unknown_order_rejected=1 global_range_reads=0 "
