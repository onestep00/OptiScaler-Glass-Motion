...
int wmain(int argc, wchar_t** argv)
{
    try
    {
        ...
        // The in-place packed target preserves the original UAV writes, so a
        // UAV-writing material is admitted instead of rejected. The GPU checks
        // below verify that the original writes survive.
        check(guardedCompiler.createPackedMotion(g.d.Get(), root, pd, patched, error));
        patched.Reset();
