...
Supply class is bit 0 root-only, bit 1 skinned (t10 bones), bit 2 preskinned (t9/b3). The catalog has class 1 and class 2 records only. The default mask 1 admits the 349 class 1 records (82 root grafts, 21 twin camera-only, 246 generic camera-only). The 247 class 2 records are off by default. A default of 3 was tried on DLL `0c5bb34e` and withdrawn: skinned NPC hair and glasses delivered about 77 px where the engine had about 3.5 px (`research/ACTIVE.md:128`). The cause was the missing MotionMatrix declaration for the `hair_basecolor_blend` pairs; with them declared (`5ca2eaba`), mask 3 measured 0.1 px on NPC hair (research/ACTIVE.md "hair 선언 쌍 게임 검증"). The default stays 1 until the user decides. The 45 refused vehicle VS (25 class 1, 20 class 2) had camera-only records until 2026-09-24.
...
| Skinned (class 2) records | 247 records, off by default | the default is a user decision. Measured 2026-09-24 with mask 3: before `5ca2eaba` the `hair_basecolor_blend` pairs had no MotionMatrix declaration, rows 24..26 stayed unwritten and NPC hair was wrong by 105–110 px; with the pairs declared (DLL `985942b5`) the hair probe reads the current pose on still NPCs and the previous pose on walkers (`rows_other=0`), and hair residuals are 0.04–0.14 px (0.28–0.49 px in frames with a close walker, whose large residuals sit on edges over the background). The skinned-current variant (current world) lacks the root motion of walkers (≈50 px) and is not used. No generated-frame A/B of walkers with mask 1 against 3 yet | `research/ACTIVE.md` "스키닝 A/B", "hair 선언 쌍 게임 검증" |
