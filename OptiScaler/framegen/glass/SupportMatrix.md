...
| Skinned (class 2) records | 247 records, off by default | measured 2026-09-24 (`009904f8`, mask 3): for a skinned draw whose proxy has an engine history record, the MotionMatrix rows 24..26 the declaration supplies are not a transform (`motionprobe`: rows neither the current nor the previous pose on every hair draw), so the root graft is wrong by 105–110 px on still NPCs. The previous bones are right: the skinned-current variant (current world, previous bones, previous camera) delivers 0.08 px on still NPC hair but lacks the root motion of walking NPCs (≈50 px on a close walker). The engine history record holds the previous pose (45–78 mm per frame on walkers) | `research/ACTIVE.md` "스키닝 A/B" |
