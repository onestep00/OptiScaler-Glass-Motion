...
| Rain (`rain`, VS `5241cee6`) on the City Center street with `24h_weather_rain`, 2026-09-24, `d8569b1f` | ownerless camera-only (camera component only; no fall motion) | 402 draws, captured/eligible 100%; about 39% of the frame (1.4M px) takes the streak motion; residual against the engine mean 0.18–0.23 px. Generated frames on/off during rotation, a short strafe and a 3.5 s run show no streak-shaped background defect. Driving speed not checked | ACTIVE.md "입자 camera-only 교정" |
