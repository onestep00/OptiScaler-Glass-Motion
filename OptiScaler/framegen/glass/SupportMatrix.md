...
| Skinned (class 2) records | 247 records, off by default | measured 2026-09-24 (`009904f8`, mask 3): on NPC hair draws the MotionMatrix supplier leaves rows 24..26 unwritten (`motionprobe`: the same non-transform bytes for every proxy in a frame, with or without history), so the root graft is wrong by 105–110 px on still NPCs. The probed skinned `glass_onesided` (`bcda39b3`) reads rows equal to the current pose; it and the other class 2 pipelines seen (`c73bfe23`, `parallaxscreen_transparent` `289d4f36`, `eye_shadow` `90ead7b2`) stay at 0.05–0.20 px. The previous bones are right: the skinned-current variant (current world, previous bones, previous camera) delivers 0.08 px on still NPC hair but lacks the root motion of walking NPCs (≈50 px on a close walker). The engine history record holds the previous pose (45–78 mm per frame on walkers) | `research/ACTIVE.md` "스키닝 A/B" |
