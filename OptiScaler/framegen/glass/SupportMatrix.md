...
- Applied: catalog `GGRAFT02` with 596 records, `index.bin` SHA-256 `c85137b6e45fb8250d31eaa53f6b77011ef8afd5f9abd665808adb0cf90dc9c3`, and `refused.bin` (`GGREFS01`, 45 vehicle VS) SHA-256 `adf808dff2ca9f347af57ab406d8c6ddf339f2a0115fda124291c5992a83c582`, for `glass-motion` builds from `764086c7` on (the 641-record catalog `52e45006…` before it). Builds with the light-pass rule also read `light-ps.bin` (`GGLTPS01`, 755 light-pass PS) SHA-256 `9e945d2a2e26920f5bd0c8fdb705ec867a81d9f542973b930fe4981ee49df7e5`. Builds with the background rule also read `background-ps.bin` (`GGBGPS01`, 152 PS) SHA-256 `a2db74537ddb21a9443080c38d3af72c1561c6c3048509fb30397250135b3fd3`. Default `GraftClassMask` 1: class 2 records are off unless enabled (`research/ACTIVE.md:128`).
...
Supply class is bit 0 root-only, bit 1 skinned (t10 bones), bit 2 preskinned (t9/b3). The catalog has class 1 and class 2 records only. The default mask 3 admits all 596 records: 349 class 1 (82 root grafts, 21 twin camera-only, 246 generic camera-only) and 247 class 2. A default of 3 was first tried on DLL `0c5bb34e` and withdrawn: skinned NPC hair and glasses delivered about 77 px where the engine had about 3.5 px (`research/ACTIVE.md:128`). The cause was the missing MotionMatrix declaration for the `hair_basecolor_blend` pairs; with them declared (`5ca2eaba`), mask 3 measured 0.1 px on NPC hair (research/ACTIVE.md "hair 선언 쌍 게임 검증"), and 3 is the default since 2026-09-24. The 45 refused vehicle VS (25 class 1, 20 class 2) had camera-only records until 2026-09-24.
...
| `global_water_patch`, `water_plane` (every PS in `background-ps.bin`: coverage only) | 4 | 1 c1 | 3 c1 | 0 | 0 |
...
| `cloak*`, `optical_camouflage` (screen-space VFX and distortion PS in `background-ps.bin`: coverage only) | 62 | 36 (9 c1, 27 c2) | 23 (21 c1, 2 c2) | 3 c1 | 0 |
...
| `global_water_patch` | 3 | 0 | 3 c1 | 0 | `transparent`, `distortion`, `transparent_mark_rt`: MeshStatic `21b0543f`; `screen_space_water_depth`: `46aa4872`, `ccea6dab`. All 9 PS of its techniques are in `background-ps.bin`: their packed variants record coverage only |
...
| Kabuki Market Data Term hologram (`parallaxscreen_transparent`, material opacity 0), 2026-09-24, `5e718785` and `009904f8` | root graft; brightness term | real strafe 4–10 m, eight generated frames per arm: emission 0 (material opacity alone) smears the map interior sideways; emission 100 with threshold 10–50 keeps the map lines sharp; the threshold 75 and 100 frames show smeared interior patches. Hologram records covered at threshold 50: 32–52% depending on the frame | ACTIVE.md "불투명도·발광 밝기 통합 판정" |
| NPC hair and glasses (class 2 skinned root grafts: `hair_basecolor_blend`, skinned `glass_onesided`, `eye_shadow`), 2026-09-24, `985942b5`, mask 3 | root graft: previous bones (t10 at `INSTANCE_SKINNING_DATA.z`), engine MotionMatrix, previous camera | before `5ca2eaba` hair read unwritten rows 24..26 and was wrong by 105–110 px. After: probe rows equal the current pose on still NPCs and the previous pose on walkers (`rows_other=0`); hair 0.04–0.14 px (0.28–0.49 px with a close walker), skinned glasses 0.4–1.05 px where their edge moves over the background. Generated frames on/off at the club show no smearing or doubling at hair or glasses edges on either | ACTIVE.md "스키닝 A/B", "hair 선언 쌍 게임 검증", "class 2 생성 프레임 A/B" |
| Same, default mask 3 (`27a78995`, `87e1c2a9`), 2026-09-24, DLL `d609865c`, INI without `GraftClassMask` | as above | `classmask=3`, `class_disabled=0`; club hair 0.08–0.13 px; Jig-Jig Street probe `rows_prev=2809 rows_other=0`. NPCs passing within about 1 m at 20–90 px/frame: where the engine shows the NPC, hair residual p50 0.3–1.3 px and p95 6.7–27.8 px on hair strands over the face or background; where it shows the background the difference is the intended object motion. Compose `gpu_ms` 0.17. The first launch ended about 12 s in with an access violation in the engine's small-block allocator (`Cyberpunk2077.exe+0x143DEE`, heap corruption, cause unknown); the second launch ran normally | ACTIVE.md "class 2 기본 활성화" |
...
- A light-pass PS (`light-ps.bin`: every census technique pass is a transparent, unlit or screen-space VFX pass) whose blend equation is analysed records the larger of its material opacity and its displayed brightness (`emission=`, default 100%; [README.md](README.md#3-capture-packed-records)). Additive and premultiplied-emissive surfaces (holograms, rain, `light_gradients`) have material opacity 0, so their interior is covered where its brightness reaches the interior threshold; dimmer interior pixels keep the engine value. The exception since 2026-09-25 is a dimmer pixel within `BorderWidthPx` of a covered pixel of the same object: it lies on the covered region's boundary band and takes the object's motion like the silhouette band, so the antialiased rim of a hologram glyph moves with its bright core instead of splitting from it under camera translation ([README.md](README.md#4-compose-glassobjectmotionhlsl)). Every other PS records its material opacity alone: G-buffer decal, mark, depth and highlight passes write no light on screen. Distortion passes write none either; their PS are in `background-ps.bin` and record coverage only (below). Without `light-ps.bin` no PS gets the brightness term (`GEOMETRY_LIGHT_PS state=missing`). In game (2026-09-24, `009904f8`): `GEOMETRY_LIGHT_PS state=loaded count=755`; the G-buffer decal (`35e8c18f`, `d2cd7ed6`, `43718f02`), distortion (`ccff8595`, `13489c05`, `a37f8ae8`) and HUD marker (`77d68c2f`) pipelines report `light=0`, the hologram and glass pipelines `light=1`. Rain records average d = 0.001 (luma(F) p90 0.012), so the rain interior keeps the engine value at any threshold above 1%.
...
- A PS listed in `background-ps.bin` (152 PS: every `distortion` PS, the `cloak`/`cloak_v2` screen-space VFX PS and every `global_water_patch`/`water_plane` PS) shows background content, not the surface it is drawn on. Every packed variant of such a PS records coverage only, light-pass PS included, so its interior keeps the engine value and only the boundary takes the object's motion (`GRAFT background=`, [README.md](README.md#background-pixel-shaders-background-psbin)). The glass, liquid, fluid and vehicle-glass families keep their transparent-pass records; only their distortion PS are listed. Before this rule the distortion draws recorded their material opacity: vehicle-glass distortion records averaged 0.255 (`research/ACTIVE.md` "불투명도·발광 밝기 통합 판정").
...
| Root-graft pairs without a MotionMatrix declaration | 6 `blood_transparent` `gbuffer_regular` pairs (writer unresolved), plus highlights, vision, wireframe, G-buffer and shadow techniques of root-graft VS | the engine writes rows 24..26 only for declared pairs. None of these passes reached the packed capture in the per-pipeline reports of 2026-09-24 (14 logs, 110 pipelines); an admitted one would read unwritten rows | research/ACTIVE.md "class 2 기본 활성화" |
