...
void save(bool manualVerified,uint64_t beforeSeen,uint64_t beforeMatched) {
    ...
    file<<"{\"enabled\":"<<enabled.load()<<",\"seen\":"<<seen.load()<<",\"matched\":"<<matched.load()
        ...
        <<",\"manual_native_provider_verified\":"<<(manualVerified || manualVerifiedOnce)<<",\"cached_layout_invalidated\":false,\"fg_input_changed\":false}";
}
}
...
int main() {
    ...
    void* returned=nullptr;enabled=true;
