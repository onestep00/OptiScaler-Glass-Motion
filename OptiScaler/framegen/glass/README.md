...
- A single-instance draw of a root-graft pipeline whose owner proxy has no engine-supplied previous pose this frame (no history record, record state above 1 or motion weight 0) and no motion flag uses the camera variant. The engine writes a previous pose into the MotionMatrix rows only for a proxy re-transformed this frame, and its velocity pass skips a rigid proxy otherwise, leaving the camera-only velocity initialization (`EngineMotionSupply.md` "Proxy history convention"). The rule is `stalemotion=camera`, the default; `stalemotion=off` keeps the root graft for A/B. Such draws count `GRAFT stale_camera`, the per-pipeline `array` counter and the dump variant `stale`.
...
| `controls`, `identity` | control word (including `stalemotion=camera|off` and `motionprobe=<hex>|off`) and identity rejection split |
