...
- `GraftClassMask` admits records by the supply class of their previous graph: bit 0 root-only, bit 1 skinned (t10), bit 2 preskinned (t9/b3). The default is 1, root only (`GlassControls.h:288-305`, `GlassSettings.cpp:80-81`). A default of 3 was tried on DLL `0c5bb34e` and withdrawn: in all six still frames the NPC head region (hair and glasses, skinned class 2) delivered about 77 px where the engine had about 3.5 px (`research/ACTIVE.md:128`). A disabled class counts `class_disabled` and keeps the engine values.
...
The skinned-current experiment (`research/ACTIVE.md:244`) passes `--skinned-previous-world current` to all three tools (same `--generic-extra`) and exports to `glass-optiscaler-source\artifacts\glass-grafts-skinned-current\Glass`. Both exports write the same `index.bin` and `refused.bin`; they differ in the class 2 root graft bytes of non-vehicle targets.
...
The record opacity d is 1 minus the largest share of the displayed pixel that the background can still influence. For the blend C = F + T·B the rewritten PS writes d = max(1 − mean(T), saturate(luma(F) · s)) (`DxilVertexHistory.cpp` `RewriteMaterialMotion`). The first term is the material opacity of the original blend equation, T being the saturated RGB transmission. The second is the displayed brightness of the light F the draw adds, as a fraction of white (Rec. 709 luma): the display clips at 1, so added light that reaches white hides the background completely. s is `MaterialCaptureConstants::emissionScale`, the live `emission=` percent over 100, default 1.0. The game passes `DLSS.Pre.Exposure` 1.0, no exposure texture and a clear auto-exposure flag, so the scene colour the transparent pass writes is already exposed (workspace `artifacts/OptiScaler-nr-preupscale-20260924.log`). Additive and premultiplied-emissive materials (holograms such as `parallaxscreen_transparent`, rain, `light_gradients`) have material opacity 0, so only the second term covers their bright interiors. The 8-bit weight and the covered class both come from d (`PackedMotionShader.h:95-104`, `128`); `emission=0` restores the opacity-only record. The rule has no in-game result yet.

The coverage-only fallback variant, used when the equation cannot be read, analyses no colour, records d = 0 and therefore contributes only its boundary unless the threshold is 0. Because the store is an unsigned max, a covered record outranks an uncovered one, and within a class the nearest surface wins (C8) (`PackedMotionShader.h:109-150`). A draw whose motion at a pixel exceeds ±128 px writes no record there (`PackedMotionShader.h:82-89`); the pixel keeps the engine value unless another surface recorded it.
